...
product-report-smoke:
	...
	python3 -c 'import json, sys; create = json.load(open(sys.argv[1])); report = json.load(open(sys.argv[2])); assert create["status"] == "planned"; assert report["status"] == "ok"; assert report["run_id"] == create["run_id"]; assert report["lifecycle"]["status"] == "planned"; assert report["seed_metadata"]["seed_id"] == "fixture:first"; assert report["b_gate"] == {"available": False, "passed": None}; print("[product-report-smoke] /runs report contract passed:", report["run_id"])' /tmp/barred-fleet-product-create.json /tmp/barred-fleet-product-report.json
...
verify-fresh-demo:
	...
	python3 -c 'import json, sys; payload = json.load(open(sys.argv[1])); gateway = payload["agent_gateway"]; assert payload["status"] == "attention_required"; assert payload["run_id"] == "fresh-demo-agent-gateway-block"; assert payload["error_category"] == "egress_policy"; assert payload["error"] == "agent gateway blocked live execution"; assert gateway["status"] == "blocked"; assert gateway["mode"] == "local_policy"; assert gateway["decision_authority"] == "routing_and_egress_only"; assert gateway["egress_decision"]["blocked"] is True; assert gateway["egress_decision"]["reason"] == "model_route_blocked"; assert gateway["model_route_policy"]["rejected_routes"] == ["unapproved/provider"]; print("[verify-fresh-demo] Agent Gateway block contract passed")' /tmp/barred-fleet-fresh-agent-gateway-block.json
