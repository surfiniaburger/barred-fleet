...
"""
        ...
        try:
            ...
            await updater.add_artifact(
                parts=[
                    Part(text=json.dumps(report.model_dump(), ensure_ascii=False)),
                    Part(text=json.dumps({"verifier_llm_usage": usage_summary}, ensure_ascii=False)),
                    Part(text=f"Verification Result: {'PASSED' if report.passes_audit else 'FAILED'}"),
                    Part(text=report.thinking_process),
                    Part(text=f"Logic Audit: {report.logic_error or 'None'}")
                ],
                name="VerifierReport",
            )
