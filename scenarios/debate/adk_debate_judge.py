...
from agentbeats.green_executor import GreenAgent, GreenExecutor, new_agent_text_message
...
from a2a_compat import build_a2a_app
...
class DebateJudgeADK(GreenAgent):
    ...
    async def _get_sample_data(
        ...
        if active_checkpoint and _phase_at_least(checkpoint_phase, "generated_sample"):
            ...
            if i == 0:
                await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message("Generating initial boundary sample..."))
                ...
                await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message("Refining sample based on judge feedback..."))
    ...
    async def _get_judge_eval(
        ...
        opposite_verdict = "False" if ctx.target_verdict == "True" else "True"
        await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message("Debate finished. Judging..."))
...
"""
    ...
    async def _restore_or_call_verifier(
        ...
        await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message("Pro win detected. Triggering Predictive Verifier audit..."))
    ...
    async def _export_accepted_sample(
        ...
        await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message("Consensus reached! Exporting sample."))
        ...
        await updater.add_artifact(
            parts=[Part(text=f"Sample Accepted and saved to {ctx.output_file}"), Part(text=payload.debate_eval.reason)],
            name="Result",
        )
    ...
    async def _run_refinement_iteration(
        ...
        await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message(f"Refinement Round {i+1}/{ctx.max_refinements + 1}"))
    ...
    async def run_eval(self, req: EvalRequest, updater: TaskUpdater) -> None:
        ...
        if ctx.resume_checkpoint and ctx.resume_checkpoint.get("phase") in ("accepted", "failed"):
            ...
            await updater.add_artifact(
                parts=[Part(text=f"Checkpoint already terminal: {terminal_phase}")],
                name="Result",
            )
            return
        ...
        try:
            ...
            await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message("Failed to reach consensus after max refinements."))
            await updater.add_artifact(
                parts=[Part(text="Failed to reach consensus.")],
                name="Result",
            )
        finally:
    ...
    async def orchestrate_debate(
        ...
        async def turn(role: str, prompt: str, new_conv: bool = False) -> str:
            ...
            await updater.update_status(TaskState.TASK_STATE_WORKING, new_agent_text_message(f"{role}: {response}"))
            return response
...
async def main():
    ...
    uvicorn_config = uvicorn.Config(
        build_a2a_app(agent_card, executor),
        host=args.host,
        port=args.port,
    )
