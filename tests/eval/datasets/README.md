...
## BARRED Artifact Gate Dataset

`barred-artifact-gate-dataset.json` is an opt-in ADK eval case for the BARRED-Fleet adapter.
It asks the agent to call deterministic artifact tools against checked-in fixture data.

Running `agents-cli eval generate` or `agents-cli eval run` may still call the configured
agent model, and the scaffolded `custom_response_quality` grader calls Gemini. Use the
unit tests for the no-live-model regression path.
